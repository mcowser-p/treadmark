# cairn — File Integrity Forensics for Linux & Windows

A cairn is a stack of stones placed by trail-walkers to mark "I was here, and the ground was solid." This tool does the same thing for your filesystem: capture a known-good baseline of file hashes, metadata, and (optionally) text content. Later, when something happens, run a scan to see exactly what changed.

cairn is a **forensic tool**, not a continuous monitor. You reach for it when:
...
If you want continuous monitoring with real-time alerting, who-data attribution, and a SIEM dashboard, run [Wazuh](https://wazuh.com/) or a commercial HIDS. cairn fits in the gap where you want a single binary, no agent, no central server, and human-readable answers.
...
Pick one — they all give you the same `cairn` command.

| Target | Artifact | Install |
|---|---|---|
| Debian / Ubuntu | `.deb` | `sudo apt install ./cairn_X.Y.Z_amd64.deb` |
| RHEL / Rocky / Fedora | `.rpm` | `sudo dnf install ./cairn-X.Y.Z-1.x86_64.rpm` |
| Locked-down Linux server | single binary | drop `cairn-linux-x86_64` on the host |
| Windows | `.msi` | `msiexec /i cairn-X.Y.Z.msi /qb` |
| Dev / pipx | wheel | `pipx install ./cairn-X.Y.Z-py3-none-any.whl` |
...
|---|---|---|
| Binary | `/usr/bin/cairn` | `C:\Program Files\Cairn\cairn.exe` |
| Config | `/etc/cairn/cairn.yaml` | `C:\ProgramData\Cairn\cairn.yaml` |
| Baseline DB | `/var/lib/cairn/baseline.db` | `C:\ProgramData\Cairn\baseline.db` |
...
cairn has exactly one operating model:
...
sudo $EDITOR /etc/cairn/cairn.yaml          # decide what to monitor
sudo cairn files init --config /etc/cairn/cairn.yaml

# Save the baseline's provenance to your runbook (chain-of-custody):
sudo cairn baseline info --config /etc/cairn/cairn.yaml > runbook/baseline-$(hostname).txt
...
`cairn files scan` writes seven formats:
...
# → dist/{*.whl, cairn-linux-*, *.deb, *.rpm}

# Windows (needs Python 3.9+ and .NET SDK 6+)
.\scripts\build-windows.ps1
# → dist/{cairn-windows-x86_64.exe, cairn-*.msi}
```
...
- [`docs/forensic-workflow.md`](docs/forensic-workflow.md) — the investigation workflow, triage tips, what cairn won't tell you
